...
\end{removedblock}
\begin{addedblock}
\tcode{U} is valid value type for optional.
\end{addedblock}
The declaration
\begin{codeblock}
...
\pnum
An instance of optional<T\&> contains a value if \tcode{val != nullptr} is \tcode{true}.
...
\begin{itemdescr}
  ...
  \item \tcode{!detail::reference_constructs_from_temporary_v<T\&, Arg>)} is \tcode{true}
  \end{itemize}

  \pnum
  \effects
  Initializes val with \tcode{addressof(static_cast<T\&>(std::forward<Arg>(arg)))}
...
\begin{itemdescr}
  ...
  \item \tcode{!(is_same_v<remove_cvref_t<U>, in_place_t>)} is \tcode{true}
  \item \tcode{!(is_same_v<remove_cvref_t<U>, optional>)} is \tcode{true}
  \end{itemize}

  \pnum
  \effects
  Initializes \tcode{val} with \tcode{addressof(static_cast<T\&>(std::forward<U>(u)))}.
...
\begin{itemdescr}
  ...
  \begin{itemize}
  \item \tcode{is_constructible_v<T\&, U\&>} is \tcode{true}
  \item \tcode{!std::is_same_v<remove_cv_t<T>, optional<U>>} is \tcode{true}
  \item \tcode{!std::is_same_v<T\&, U>} is \tcode{true}
  \end{itemize}
...
\begin{itemdescr}
  ...
  \pnum
  \effects
  Assigns \tcode{val} \tcode{addressof(static_cast<T\&>(std::forward<U>(u)))}
...
\begin{itemdescr}
  ...
  \begin{itemize}
    \item\tcode{!is_same_v<remove_cvref_t<U>, in_place_t>} is \tcode{true}
    \item\tcode{!is_same_v<remove_cvref_t<U>, nullopt_t>} is \tcode{true}
    \item\tcode{(is_object_v<U> \&\& !is_array_v<U>) || is_lvalue_reference_v<U>} is \tcode{true}
  \end{itemize}
...
\begin{itemdescr}
  ...
  val = nullptr;
  \end{codeblock}
  \tcode{val} does not refer to a value.
\end{itemdescr}
...
\added{or}
\added{\tcode{is_reference_v<T>} is \tcode{true}}
