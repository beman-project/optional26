...
namespace std {
  ...
  @\added{// \ref{optional.optionalrref}, class template \tcode{optional} for rvalue reference types }@
  @\added{template <class T> }@
  @\added{class optional<T\&\&>; // partially freestanding}@
...
\begin{addedblock}
\rSec2[optional.optionalref]{Class template \tcode{optional} \added{for reference types}}
...
namespace std {
  ...
  class optional<T&> {
    ...
    public:
      ...
      template <class U = T>
        requires(!@\exposconcept{is-derived-from-optional}@<decay_t<U>>)
      ...
      template <class U> constexpr optional& operator=(optional<U>&& rhs) = delete;
      template <class U>
        requires(!@\exposconcept{is-derived-from-optional}@<decay_t<U>>)
...
\rSec2[optional.optionalrref]{Class template \tcode{optional} \added{for rvalue reference types}}

\rSec3[optional.optionalrref.general]{General}
\begin{codeblock}
namespace std {
  template <class T>
  class optional<T&&> {
    public:
      using value_type     = T&&;
      using iterator       = @\impdefnc@; // see~\ref{optionalrref.iterators}
      using const_iterator = @\impdefnc@; // see~\ref{optionalrref.iterators}
    ...
    public:
      ...
      // \ref{optionalrref.mod}, modifiers
      constexpr void reset() noexcept;
...
\begin{itemdescr}
    ...
    \mandates
    \begin{itemize}
    \item \tcode{is_constructible_v<add_rvalue_reference_t<T>, U>} is \tcode{true}
    \end{itemize}
...
\begin{itemdescr}
  ...
  \item \tcode{is_convertible_v<decltype(u), T>} is \tcode{true}
  \end{itemize}
  \pnum
  \effects
  Equivalent to:
  \tcode{return has_value() ? std::move(*val) : std::forward<U>(u);}
\end{itemdescr}
...
\begin{itemdescr}
  ...
  Equivalent to:
  \begin{codeblock}
    return has_value() ? invoke(forward<F>(f), value()) : remove_cvref_t<U>();
  \end{codeblock}
\end{itemdescr}
...
\begin{itemdescr}
  ...
  If \tcode{*this} refers to a value, an \tcode{optional<U>} object
  whose refered to value is the result of
  \tcode{invoke(forward<F>(f), std::move(*val))};
  otherwise, \tcode{optional<U>()}.
\end{itemdescr}
...
\rSec3[optionalrref.mod]{Modifiers}
